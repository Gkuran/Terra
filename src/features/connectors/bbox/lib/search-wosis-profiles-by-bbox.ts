...
export interface WosisBoundingBoxSearchResult {
  featureCollection: FeatureCollection<Geometry, FeatureProperties>
  queryLabel: string
  resultCount: number
}

export async function searchWosisProfilesByBbox(
  bbox: MapBoundingBox,
): Promise<WosisBoundingBoxSearchResult> {
  if (!env.VITE_API_BASE_URL) {
    throw new Error('VITE_API_BASE_URL is not configured for WoSIS bbox searches.')
  }

  const response = await fetch(`${env.VITE_API_BASE_URL}/api/v1/soil-profiles/search-by-bbox`, {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) {
    ...
    const detail =
      ...
      typeof errorPayload.detail === 'string'
        ? errorPayload.detail
        : 'WoSIS bounding box search failed.'
